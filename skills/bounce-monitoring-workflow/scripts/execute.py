...
# ///
"""
Bounce Monitoring Workflow — Execute
...
# ── Configuration ────────────────────────────────────────────────
# Bounce count that triggers suppression (2-3 typical)
SUPPRESSION_THRESHOLD = 2
# Bounce count that flags a contact for weekly manual review (3-5 typical)
REVIEW_THRESHOLD = 3
# Custom property used by the review process (created if missing)
FLAG_PROPERTY = "email_health_flag"

SKILL_SLUG = "bounce-monitoring-workflow"
SKILL_TITLE = "Bounce Monitoring"
...
FLOWS = [
    contact_flow(
        name="BOUNCE: Hard Bounce Suppression (API)",
        filters=[prop_known("hs_email_hard_bounce_reason_enum")],
        actions=[
            set_property_action(1, FLAG_PROPERTY, "true", next_id=2),
            set_marketing_status_action(2, marketable=False),
        ],
    ),
    contact_flow(
        name=f"BOUNCE: Suppress at {SUPPRESSION_THRESHOLD}+ Bounces (API)",
        filters=[prop_gte("hs_email_bounce", SUPPRESSION_THRESHOLD)],
        actions=[
            set_marketing_status_action(1, marketable=False),
        ],
        re_enroll=True,
    ),
    contact_flow(
        name=f"BOUNCE: Flag for Review at {REVIEW_THRESHOLD}+ Bounces (API)",
        filters=[prop_gte("hs_email_bounce", REVIEW_THRESHOLD)],
        actions=[
            set_property_action(1, FLAG_PROPERTY, "true"),
        ],
        re_enroll=True,
    ),
]
...
def ensure_flag_property():
    """Create the bounce review flag property if it does not exist."""
    ...
    resp = api("POST", "/crm/v3/properties/contacts", json={
        "name": FLAG_PROPERTY,
        "label": "Email Health Flag",
        "type": "bool",
        "fieldType": "booleancheckbox",
        "groupName": "contactinformation",
        "description": "Flagged for bounce review. Managed by the BOUNCE workflows; "
                       "cleared manually after weekly review.",
        "options": [
