...
# ///
"""
Engagement Suppression Workflow — Execute
...
# ── Configuration ────────────────────────────────────────────────
# Sunset window: days without an email open before re-engagement (120-270 typical)
SUNSET_DAYS = 180
# Re-engagement window: days given to respond before suppression (21-45 typical)
REENGAGE_DAYS = 30
# Custom dropdown property that tracks sunset state (created if missing)
FLAG_PROPERTY = "engagement_flag"

SKILL_SLUG = "engagement-suppression-workflow"
SKILL_TITLE = "Engagement Suppression"
...
FLOWS = [
    contact_flow(
        name=f"SUNSET: Tier 1 - Flag for Re-engagement ({SUNSET_DAYS}d, API)",
        filters=[
            prop_older_than("hs_email_last_open_date", SUNSET_DAYS, include_never=True),
            prop_known("email"),
            prop_not_true("hs_email_optout"),
            prop_unknown(FLAG_PROPERTY),
        ],
        actions=[
            set_property_action(1, FLAG_PROPERTY, "re_engagement_sent"),
        ],
    ),
    contact_flow(
        name=f"SUNSET: Tier 2 - Suppress Non-responders (+{REENGAGE_DAYS}d, API)",
        filters=[
            prop_eq_enum(FLAG_PROPERTY, "re_engagement_sent"),
            prop_older_than("hs_email_last_open_date", SUNSET_DAYS + REENGAGE_DAYS,
                            include_never=True),
            prop_older_than("hs_email_last_click_date", SUNSET_DAYS + REENGAGE_DAYS,
                            include_never=True),
        ],
        actions=[
            set_property_action(1, FLAG_PROPERTY, "suppressed", next_id=2),
            set_marketing_status_action(2, marketable=False),
        ],
    ),
]


def ensure_flag_property():
    """Create the sunset flag property if it does not exist."""
    resp = api("GET", f"/crm/v3/properties/contacts/{FLAG_PROPERTY}")
    if resp.status_code == 200:
        print(f"Property '{FLAG_PROPERTY}' exists.")
        return
    print(f"Creating contact property '{FLAG_PROPERTY}'...")
    resp = api("POST", "/crm/v3/properties/contacts", json={
        "name": FLAG_PROPERTY,
        "label": "Engagement Flag",
        "type": "enumeration",
        "fieldType": "select",
        "groupName": "contactinformation",
        "description": "Two-tier sunset state. Managed by the SUNSET workflows.",
        "options": [
            {"label": "Re-engagement sent", "value": "re_engagement_sent",
             "displayOrder": 0},
            {"label": "Suppressed", "value": "suppressed", "displayOrder": 1},
        ],
    })
    resp.raise_for_status()
