...
# ///
"""
Lifecycle Progression Workflow — Execute
...
# ── Configuration ────────────────────────────────────────────────
# The score property from your lead scoring model (see build-lead-scoring).
# HubSpot's post-2025 scoring tool generates a property per score you create;
# find the internal name under Settings > Properties.
LEAD_SCORE_PROPERTY = "hs_v2_engagement_score"  # CHANGE to your score property
MQL_SCORE_THRESHOLD = 50

SKILL_SLUG = "lifecycle-progression-workflow"
SKILL_TITLE = "Lifecycle Progression"
...
FLOWS = [
    contact_flow(
        name="LIFECYCLE: Lead to MQL on Score (API)",
        filters=[
            prop_gte(LEAD_SCORE_PROPERTY, MQL_SCORE_THRESHOLD),
            prop_eq_enum("lifecyclestage", "lead"),
        ],
        actions=[set_property_action(1, "lifecyclestage", "marketingqualifiedlead")],
    ),
    contact_flow(
        name="LIFECYCLE: MQL to SQL on Meeting (API)",
        filters=[
            prop_known("engagements_last_meeting_booked"),
            prop_eq_enum("lifecyclestage", "marketingqualifiedlead"),
        ],
        actions=[set_property_action(1, "lifecyclestage", "salesqualifiedlead")],
    ),
    contact_flow(
        name="LIFECYCLE: SQL to Opportunity on Deal (API)",
        filters=[
            prop_gte("num_associated_deals", 1),
            prop_eq_enum("lifecyclestage", "salesqualifiedlead"),
        ],
        actions=[set_property_action(1, "lifecyclestage", "opportunity")],
    ),
    contact_flow(
        name="LIFECYCLE: Opportunity to Customer on Closed-Won (API)",
        filters=[
            # hs_current_customer is a read-only system property (June 2026)
            # set by HubSpot when the contact belongs to a current customer.
            prop_bool("hs_current_customer", True),
            prop_eq_enum("lifecyclestage", "opportunity"),
        ],
        actions=[set_property_action(1, "lifecyclestage", "customer")],
    ),
]
